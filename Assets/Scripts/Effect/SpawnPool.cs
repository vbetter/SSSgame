/// <Licensing>
/// 锟?2011 (Copyright) Path-o-logical Games, LLC
...
//    public PrefabsDict prefabs = new PrefabsDict();
    ...
    private void OnDestroy()
    {
        if (this.logMessages)
		{
			Debug.LogWarning(string.Format("SpawnPool {0}: Destroying...", this.poolName));
		}
...
//        this.prefabs._Clear();
	...
	public List<Transform> CreatePrefabPool(PrefabPool prefabPool , string prefabName = "")
    {
...
//            this.prefabs._Add(prefabPool.prefab.name, prefabPool.prefab);
        ...
        if (prefabPool.preloaded != true)
        {
			...
			instances.AddRange(prefabPool.PreloadInstances(prefabName));
        }
    ...
    {
		#if Profile
		Profiler.BeginSample("SpawnPool.Spawn");
		#endif
        Transform inst;
    ...
    {
		#if Profile
		Profiler.BeginSample("SpawnPool.Despawn");
		#endif
    ...
    private IEnumerator DoDespawnAfterSeconds(Transform instance, float seconds, PrefabPool prefabPool)
    {
        ...
        this.Despawn(instance, prefabPool);
    }
    ...
    public bool IsDespawned(Transform instance,PrefabPool prefabPool)
    {
        return prefabPool.despawned.Contains(instance);
    }
...
public class PrefabPool
{
    ...
    internal bool DespawnInstance(Transform xform, bool setInActive = true, bool callDeSpawned = true)
    {
        if (this.logMessages)
		{
			Debug.Log(string.Format("SpawnPool {0} ({1}): Despawning '{2}' active({3})",
			                                      ...
			                                      this.prefab.name,
				xform.name, xform.gameObject.activeInHierarchy));
		}
		...
		if(callDeSpawned == true) {
			if(xform.gameObject.activeInHierarchy == false) {
    ...
    public void ClearExcessPrefab() 
    {
        while (this.totalCount > this.limitAmount)
        {
    ...
    /// </summary>
    /// <returns></returns>
	internal List<Transform> PreloadInstances(string prefabName = "")
    {
        ...
        if (this.prefab == null)
        {
			Debug.Log(string.Format("SpawnPool {0} ({1}): Prefab cannot be null.",
                                         this.spawnPool.poolName,
                                         this.prefab.name));
        ...
        while (this.totalCount < this.preloadAmount) // Total count will update
        {
			...
			{
				inst.gameObject.name = prefabName;
			}
